...
if (!ctype_alpha($_POST["IncorporatedOn_36"])) {  $ErrorMsg .= "\nIncorporated On must have only alphabets;<br>";  $valid = false; } else { $IncorporatedOn_36= $_POST["IncorporatedOn_36"]; }
		if ($valid) {
			...
			$q = $pdo->prepare("UPDATE dyn_tbl_menu_1_submenu_1 SET CompanyName_64 = ?,RegistrationNumber_66 = ?,TAXRegn_79 = ?,TAN_89 = ?,IncorporatedOn_36 = ? WHERE id= ?");
			$q->execute(array($CompanyName_64,$RegistrationNumber_66,$TAXRegn_79,$TAN_89,$IncorporatedOn_36,$ID));
			Database::disconnect();
...
$pdo = Database::connect();
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$result = $pdo->prepare("SELECT ID,CompanyName_64,RegistrationNumber_66,TAXRegn_79,TAN_89,IncorporatedOn_36 FROM dyn_tbl_menu_1_submenu_1 WHERE id= :id");
...
$ID = $row['ID'];
$CompanyName_64 = $row["CompanyName_64"];
$RegistrationNumber_66 = $row["RegistrationNumber_66"];
$TAXRegn_79 = $row["TAXRegn_79"];
$TAN_89 = $row["TAN_89"];
$IncorporatedOn_36 = $row["IncorporatedOn_36"];
