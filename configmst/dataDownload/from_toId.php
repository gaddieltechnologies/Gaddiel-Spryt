<?php
				...
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);				
				$results = $pdo->prepare("SELECT * FROM ".$tableName." WHERE id>=$txtFromIdname AND id<= $txtdoIdname");				
				$results->execute();
				Database::disconnect();
			...
			foreach ($csv as $row) {
				...
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$sql = "INSERT INTO download_log (Download_date,Download_filename,Table_name,Start_Id,End_Id,Last_Id) values(now(), ?, ?, ?, ?, ?)";
				$q = $pdo->prepare($sql);
				$q->execute(array($fileNames,$tableName,$txtFromIdname,$txtdoIdname,$i));
				Database::disconnect();
				exit;	
?>				