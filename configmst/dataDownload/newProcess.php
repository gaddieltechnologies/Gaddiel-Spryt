<?php
				...
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);				
				$results = $pdo->prepare("SELECT * FROM ".$tableName." WHERE id <='$txtprocessIdNam'");
				$results->execute();
				Database::disconnect();
			...
			foreach ($csv as $row) {
				...
				$pdo = Database::connect();
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$sql = "INSERT INTO download_log (Download_date,Download_filename,Table_name,Last_Id) values(now(), ?, ?, ?)";
				$q = $pdo->prepare($sql);
